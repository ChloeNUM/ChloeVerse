...
function snapshotViewer() {
    ...
    async function fetchPosts(actorHandle) {
        ...
        try {
            ...
            function getNum(item) {
                return parseInt(
                    item.post.record.text.replace("DAY ", "").split(" - ")[0]
                );
            }
            ...
            const filtered = sorted.filter((item) => {
                ...
                if (!item.post.record.text.startsWith("DAY") || item.post.author.handle !== "artifyber.xyz") return false;
                if (todo_fix.includes(parseInt(item.post.record.text.replace("DAY ", "").replaceAll("\n", " - ").split(" - ")[0]))) return false;
                return true;
            })
    ...
    function displayPosts(feed, reset = false) {
        ...
        feed.forEach(item => {
            ...
            const [num, title] = text.replace("DAY ", "").replaceAll("\n", " - ").split(" - ");
            added = true;
